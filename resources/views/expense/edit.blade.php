...
@push('scripts')
  <script>
    ...
    function edit(e, id) {
      e.preventDefault();
      expenseId = id;
      $('#formEdit').attr('action', 'expense/' + id);
      swal({
        title: 'Info',
        text: 'Getting data....',
        button: false,
      ...
      axios.get('/expense/' + id).then(function(response) {
        var expense = response.data;
        $('#product_name2').val(expense['product_name']);
        $('#product_price2').val(expense['product_price']);
        $('#qty2').val(expense['qty']);
        $('#price_total2').val(expense['price_total']);
        $('#buyer_name2').val(expense['buyer_name']);
        $('#buyer_address2').val(expense['buyer_address']);
    ...
    function update(e) {
      ...
      axios.post('/expense/' + expenseId, data).then(function(response) {
